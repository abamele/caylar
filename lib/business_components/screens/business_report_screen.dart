...
class BusinessReportScreen extends StatefulWidget {
  ...
  BusinessReportScreen(
      ...
      : super(key: key);
  @override
  State<BusinessReportScreen> createState() => _BusinessReportScreen();
}

class _BusinessReportScreen extends State<BusinessReportScreen> {
                      ...
                      OutlinedButton(
                          ...
                          onPressed: () {
                            if (dateTime1 == null) {
                              ...
                              // getReport(
                              //   "${dateTime1!.year}-${dateTime1!.month}-${dateTime1!.day}","${dateTime2!.year}-${dateTime2!.month}-${dateTime2!.day}"
                              // );
                              reportStart =
                                  dateTime1.toString().substring(0, 10);
                            ...
                            return DataTable(
                              ...
                              rows: List.generate(_reports.length, (index) {
                                ...
                                double productPrice = _reports[index]
                                            ...
                                            .runtimeType ==
                                        int
                                    ? _reports[index]["productPrice"].toDouble()
                                ...
                                return DataRow(
                                    ...
                                    cells: [
                                      ...
                                      DataCell(Container(
                                          width: 80, child: Text(productName))),
                                      DataCell(Container(
  ...
  Future getReport(int dealerIndex, int employeeIndex, String reportStart,
      String reportEnd) async {
    String dealerId = "";
    String employeeId = "";
    if (employeeIndex == 0 && dealerIndex > 0) {
      dealerId = widget.dealerList[dealerIndex - 1]["id"];
    } else if (employeeIndex > 0 && dealerIndex > 0) {
      employeeId = widget.employeeList[employeeIndex - 1]["id"];
      dealerId = widget.dealerList[dealerIndex - 1]["id"];
    } else if (employeeIndex > 0 && dealerIndex == 0) {
      employeeId = widget.employeeList[employeeIndex - 1]["id"];
    }
    print("$dealerIndex vvvvvvvvvvvvvvvvvvv   $dealerId");
    print("$employeeIndex vvvvvvvvvvvvvvvvvvv   $employeeId");
    try {
      var response = await Dio().post(AppUrl.companyReport,
